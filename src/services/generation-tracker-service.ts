...
function cleanupExpiredEntries(): void {
  ...
  activeGenerations.forEach((entry, key) => {
    if (!isEntryValid(entry)) {
      ...
      logger.debug(`🧹 [Generation Tracker] Auto-expiring stale generation: ${key} (started ${entry.startedAt.toISOString()})`);
    }
  });
...
export function markScopingGenerating(): void {
  ...
  logger.debug('🚀 [Generation Tracker] Scoping generation started');
}

export function markScopingComplete(): void {
  activeGenerations.delete('scoping');
  logger.debug('✅ [Generation Tracker] Scoping generation complete');
}
...
export function markSpecPhaseGenerating(projectId: string, phase: string): void {
  ...
  logger.debug(`🚀 [Generation Tracker] Spec phase generation started: ${projectId}/${phase}`);
}

export function markSpecPhaseComplete(projectId: string): void {
  const key = `spec:${projectId}`;
  activeGenerations.delete(key);
  logger.debug(`✅ [Generation Tracker] Spec phase generation complete: ${projectId}`);
}
...
export function markBreakdownGenerating(projectId: string): void {
  ...
  logger.debug(`🚀 [Generation Tracker] Breakdown generation started: ${projectId}`);
}

export function markBreakdownComplete(projectId: string): void {
  const key = `breakdown:${projectId}`;
  activeGenerations.delete(key);
  logger.debug(`✅ [Generation Tracker] Breakdown generation complete: ${projectId}`);
}
