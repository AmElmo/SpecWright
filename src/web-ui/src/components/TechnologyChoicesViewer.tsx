...
interface TechnologyChoicesViewerProps {
  content: string;
}
...
export function TechnologyChoicesViewer({ content }: TechnologyChoicesViewerProps) {
  const [data] = useState<TechnologyChoicesData>(() => {
    try {
  ...
  if (!hasDecisions) {
    return (
      <Card className="bg-white border-slate-200 p-12 text-center">
        ...
        <p className="text-slate-600 text-lg">No technology decisions available</p>
      </Card>
    );
  }

  const activeDecision = data.technology_decisions![activeTabIndex];
                ...
                <div
                  ...
                  className={`rounded-xl border-2 transition-all overflow-hidden ${
                    isSelected
                      ...
                      : isRecommended
                        ? 'border-sky-200 bg-gradient-to-br from-sky-50/30 to-white'
